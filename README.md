...
- **v0.2.3** — fix: CLI entrypoint was a silent no-op after `npm install -g` on macOS/Linux (symlink broke the `import.meta.url` self-check); also fix Windows path mismatch by using `pathToFileURL` for cross-platform correctness. Thanks to [@dafang](https://github.com/dafang) for the report and initial fix ([#1](https://github.com/dlgod7/image-context-cascade/issues/1), [#2](https://github.com/dlgod7/image-context-cascade/pull/2)).
